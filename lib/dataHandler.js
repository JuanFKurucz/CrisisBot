...
function loadGuerraClanesData(callback){
  readFile(__dirname+"/../data/guildWarData.json",function(data){
    callback(JSON.parse(data));
  });
}

function saveGuerraClanesData(data){
  writeFile(__dirname+"/../data/guildWarData.json",JSON.stringify(data));
}
  ...
  getAllNicks(function(names){
    ...
    for(var key in newDic){
      for(var i=0;i<dic[key].length;i++){
        ...
        var a = s.split(" ");
        dic[key][i]=a[0]+" "+CloserMatch(purifyText(a[1]),names,"") + " "+a[2]+" "+a[3];
      }
    }
    callback(newDic);
  });
...
module.exports = {
    ...
    saveGuerraClanesData: saveGuerraClanesData,
    loadGuerraClanesData: loadGuerraClanesData
};
